...
def engineer_features(df: pd.DataFrame, address: str) -> pd.DataFrame:
    if df.empty:
        return pd.DataFrame([{
            ...
            'early_outflow_ratio_pct': 0,
            'young_wallet_high_activity': 0
        }])
    ...
    early_outflow = df[(df['timeStamp'] < df['timeStamp'].min() + pd.Timedelta(days=30)) & (~df['is_incoming'])]['value'].sum()
    early_outflow_ratio = (early_outflow / total_out_eth * 100) if total_out_eth > 0 else 0

    features = {
        'tx_count': tx_count,
        'wallet_age_days': age_days,
        'tx_density': tx_count / age_days,
        'total_value_out_eth': total_out_eth,
        'outflow_per_day': total_out_eth / age_days,
        ...
        'early_outflow_ratio_pct': early_outflow_ratio,
